...
from databaseManager import Log
...
def processRequest(req):
    ...
    if intent == 'search_state':
        ...
        with open('indian_states.txt', 'r') as file:
            state_list = file.read().splitlines()
        if state not in state_list:
            logger.info(state + " not a valid indian state name")
            return {
                "fulfillmentMessages": [
                    {
                        "text": {
            ...
            if state_data:
                ...
                if test_data:
                    ...
                    logger.info(intent + " unavailable")
                    return {
                        "fulfillmentMessages": [
                            {
                                "text": {
                                    "text": [
                                        "Sorry we are not able to process your query now. Please try again."
                                    ]
        ...
        with open('indian_districts.txt', 'r') as file:
            district_list = file.read().splitlines()
        if district not in district_list:
            logger.info(district + " not a valid indian district name")
            return {
                "fulfillmentMessages": [
                    {
                        "text": {
                            "text": [
                                "Please enter a valid Indian district name."
                            ]
        ...
        if today_summary:
            ...
            Log().saveConversations(session_id, query_text, None, webhook_response, intent, db)
            logger.info(intent + " executed SUCCESSFULLY")
            return {
                "fulfillmentMessages": [
                    {
        ...
        if state not in state_list:
            ...
            if zone_data:
                ...
                Log().saveConversations(session_id, query_text, state, webhook_response, intent, db)
                logger.info(intent + " executed SUCCESSFULLY")
                return {
                    "fulfillmentMessages": [
                        {
                ...
                return {
                    "fulfillmentMessages": [
                        ...
                        },
                        {
                            "text": {
                                "text": [
                                    'You can also see the summary of any Indian state by entering '
                                    '"summary of state_name"''For eg. summary of delhi.'
                                ]
                        ...
                        },
                        {
                            ...
                            "text": {
                                "text": [
                                    "Do you to see summary of any Indian state?"
                                ]
                            }
                        }
                    ]
    ...
    elif intent == "Welcome" or intent == "continue_conversation" or intent == "end_conversation" or intent == "faq" \
            or intent == "Fallback":
        logger.info("intent - " + intent)
